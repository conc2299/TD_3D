...
void FFT_3D::init()
{
  ...
  for(int i=0; i<binCntX_; i++) {
    ...
    for(int j=0; j<binCntY_; j++) {
      ...
      for(int k=0; k<binCntZ_; k++) {
        binDensity_[i][j][k] 
          = electroPhi_[i][j][k]
          = electroForceX_[i][j][k] 
          = electroForceY_[i][j][k] 
          = electroForceZ_[i][j][k] 
          = 0.0f;  
      }
    }
  }

  csTable_.resize( std::max({binCntX_, binCntY_, binCntZ_}) * 3 / 2, 0 );
  ...
  workArea_.resize( round(sqrt(std::max({binCntX_, binCntY_, binCntZ_})/2)) + 2, 0 );

  for(int i=0; i<binCntX_; i++) {
    wx_[i] = REPLACE_FFT_PI * static_cast<float>(i) 
      / (static_cast<float>(binCntX_) * static_cast<float>(binSizeX_));
    wxSquare_[i] = wx_[i] * wx_[i]; 
  }
  ...
  for(int i=0; i<binCntZ_; i++) {
    wz_[i] = REPLACE_FFT_PI * static_cast<float>(i)
      / (static_cast<float>(binCntZ_) * static_cast<float>(binSizeZ_));
    wzSquare_[i] = wz_[i] * wz_[i];
  }
}

FFT_3D::FFT_3D(int binCntX, int binCntY, int binCntZ, float binSizeX, float binSizeY, float binSizeZ)
...
void
FFT_3D::updateDensity(int x, int y, int z, float density) {
  binDensity_[x][y][z] = density;
}

std::tuple<float,float,float>
FFT_3D::getElectroForce(int x, int y, int z) const {
  std::make_tuple(
...
float
FFT_3D::getElectroPhi(int x, int y, int z) const {
...
FFT_3D::doFFT() {
  ...
  for(int i = 0; i < binCntX_; i++) {
    for(int j = 0; j < binCntY_; j++) {
      binDensity_[i][j][0] *= 0.5;
    }
    for(int k = 0; k < binCntZ_; k++) {
      binDensity_[i][0][k] *= 0.5;
    }
  }

  for(int j = 0; j < binCntY_; j++) {
    for(int k = 0; k < binCntZ_; k++) {
      binDensity_[0][j][k] *= 0.5;
    }
  }
  ...
  for(int i = 0; i < binCntX_; i++) {
    for(int j = 0; j < binCntY_; j++) {
      for(int k = 0; k < binCntZ_; k++) {
      binDensity_[i][j][k] *= 8.0 / binCntX_ / binCntY_ / binCntZ_;
    }
  }
  ...
  ddct3d(binCntX_, binCntY_, binCntZ_, 1, 
      electroPhi_, NULL, 
      (int*) &workArea_[0], (float*) &csTable_[0]);
  ddscct3d(binCntX_, binCntY_, binCntZ_, 1, 
      electroForceX_, NULL, 
      (int*) &workArea_[0], (float*) &csTable_[0]);
  ddcsct3d(binCntX_, binCntY_, binCntZ_, 1, 
      electroForceY_, NULL, 
      (int*) &workArea_[0], (float*) &csTable_[0]);
  ddccst3d(binCntX_, binCntY_, binCntZ_, 1, 
      electroForceZ_, NULL, 
      (int*) &workArea_[0], (float*) &csTable_[0]);
  }
}
